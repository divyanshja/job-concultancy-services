...
void findbestmatch()
{
	int x[4][4],y[4][4],i,j;
	 ...
	 for(i=0;i<4;i++)
	 {
	  ...
	  for(int i=0;i<4;i++)
	  {
	   		cout<<"Company name is "<<cmp[i].cmp_name<<" ,Id is "<<cmp[i].cmp_id<<endl;
	   		//<<""<<emp[cmp[i].prefer].emp_name<<endl;
	  }
	  exit(0);
}
...
void jobbycategory()
{
	...
	//cout<<temp_point[i]<<" ";
	applycat();
	double res_point=applycat1();//here to calculate the resume point
	system("cls");//for system clear
...
void fun()
{
	...
	cout<<""<<endl;
	cout<<""<<endl;
	cout<<"**************************************JOB CONSULTANCY SYSTEM***************************************"<<endl<<endl<<endl;
	cout<<"For know about admin login press 1 "<<endl;
	cout<<"For login press 0"<<endl;
	cin>>o;
	if(o==1)
	adminlogin();
	else
